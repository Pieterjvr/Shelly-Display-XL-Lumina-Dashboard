...
const cssPath = 'styles/fonts/lumina.css';
...
fs.writeFileSync('styles/fonts/lumina.css', css);

let pending = downloads.length;
if (pending === 0) console.log('No downloads needed');
downloads.forEach(dl => {
  ...
  https.get(dl.url, function(response) {
    ...
    file.on('finish', function() {
      file.close();
      pending--;
      if (pending === 0) console.log('All lumina downloads finished');
    });
  });
});
